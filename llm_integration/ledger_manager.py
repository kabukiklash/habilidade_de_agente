...
class LedgerManager:
    ...
    def _create_schema_v2(self, conn):
        conn.execute("""
    ...
    def record_event(self, event_type: str, payload: Dict[str, Any], justification: str, correlation_id: Optional[str] = None, actor_id: str = "ANTIGRAVITY"):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                INSERT INTO audit_ledger 
                (event_id, correlation_id, session_id, host_id, process_id, actor_id, event_type, payload_raw, payload_c14n, payload_hash, justification, prev_hash, current_hash, sig)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (event_id, correlation_id, self.session_id, self.host_id, self.process_id, actor_id, event_type, payload_raw, payload_c14n, payload_hash, justification, prev_hash, current_hash, sig))
            conn.commit()
            return event_id
